...
export function configIconsPlugin(configPath) {
  ...
  async function buildIconModule() {
    ...
    for (const name of uniqueNames) {
      ...
      if (!parsed) {
        const paths = references
          ...
          .map((ref) => ref.path);
        errors.push(
          `Invalid ICON_NAME "${name}" (at ${paths.join(", ")}): expected ` +
    ...
    for (const [name, { pkg, exportName }] of parsedByName) {
      ...
      if (!mod) {
        errors.push(`Unknown react-icons pack "${pkg}" (at ${paths().join(", ")}).`);
        continue;
      }
      if (!(exportName in mod)) {
        errors.push(
          `Unknown icon "${exportName}" in pack "${pkg}" (at ${paths().join(", ")}).`
        );
        continue;
      }
      if (!namesByPackage.has(pkg)) namesByPackage.set(pkg, []);
